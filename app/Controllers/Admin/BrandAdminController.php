...
class BrandAdminController extends AdminBaseController
{
    ...
    /** Upload brand logo file, return public URL (or fall back to provided URL string). */
    private function resolveLogoUrl(?string $urlField): ?string
    {
        $file = $this->request->getFile('logo_file');
        if ($file && $file->isValid() && !$file->hasMoved()) {
            $dir = FCPATH . 'assets/images/brands/';
            if (!is_dir($dir)) mkdir($dir, 0755, true);
            $ext  = $file->getClientExtension();
            $name = url_title(pathinfo($file->getClientName(), PATHINFO_FILENAME), '-', true) . '-' . time() . '.' . $ext;
            $file->move($dir, $name);
            return base_url('assets/images/brands/' . $file->getName());
        }
    ...
    public function store()
    {
        ...
        $this->db->table('brands')->insert([
            'name'              => $name,
            'slug'              => $slug,
            'logo'              => $this->resolveLogoUrl($post['logo_url'] ?? null),
            'website'           => $post['website'] ?? null,
            'description'       => $post['description'] ?? null,
            'short_description' => $post['short_description'] ?? null,
            'country_of_origin' => $post['country'] ?? null,
