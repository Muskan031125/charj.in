...
class VehicleAdminController extends AdminBaseController
{
    ...
    public function store()
    {
        ...
        $this->db->table('vehicles')->insert([
            ...
            // Performance
            'top_speed'              => !empty($post['top_speed']) ? (int) $post['top_speed'] : null,
            'acceleration_0_100'     => !empty($post['acceleration_0_100']) ? (float) $post['acceleration_0_100'] : null,
            // Charging
            'charging_time_normal'   => $post['charging_time_normal'] ?? null,
            'charging_time_fast'     => $post['charging_time_fast'] ?? null,
            'fast_charging_supported'=> !empty($post['fast_charging_supported']) ? 1 : 0,
            'fast_charging_type'     => $post['fast_charging_type'] ?? null,
            // Motor
            'motor_power'            => !empty($post['motor_power']) ? (float) $post['motor_power'] : null,
            'motor_torque'           => !empty($post['motor_torque']) ? (float) $post['motor_torque'] : null,
            // Warranty
            ...
            'kerb_weight'            => !empty($post['kerb_weight']) ? (int) $post['kerb_weight'] : null,
            'ground_clearance'       => !empty($post['ground_clearance']) ? (int) $post['ground_clearance'] : null,
            'boot_space'             => !empty($post['boot_space']) ? (int) $post['boot_space'] : null,
    ...
    private function resolveImageUrl(?string $urlField): ?string
    {
        ...
        if ($file && $file->isValid() && !$file->hasMoved()) {
            ...
            $file->move($dir, $name);
            return base_url('assets/images/vehicles/' . $name);
        }
