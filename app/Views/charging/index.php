...
@media (hover: none) {
  .station-card:active{transform:scale(.97);transition:transform .1s ease;}
}
...
<?php
$activeCityFilter = strtolower($activeCity ?? '');
if ($activeCityFilter === '') $activeCityFilter = 'all';

// $cities comes from the controller as [['city'=>'Delhi','station_count'=>12], ...], sorted by count desc
// If the active city isn't in that DB-aggregated list (e.g. it only has live/API stations, no DB rows yet),
// prepend it so the filter pill row always shows which city is actually active — avoids the confusing
// "3 stations in Chennai" heading with no matching/highlighted "Chennai" pill in the row below.
$cityNames = array_map(fn($c) => strtolower(trim($c['city'] ?? '')), $cities ?? []);
if ($activeCityFilter !== 'all' && !in_array($activeCityFilter, $cityNames, true)) {
    array_unshift($cities, ['city' => ucfirst($activeCityFilter), 'station_count' => 0]);
}
...
<!-- ═══ HERO ═══ -->
<section class="hero-sm hero-bg relative overflow-hidden pt-20 md:pt-32 pb-10 px-4">
...
<!-- ═══ CITY FILTER ═══ -->
<div class="sticky top-16 md:top-28 z-30 bg-white border-b border-slate-100 shadow-sm">
  <div class="max-w-6xl mx-auto px-4">
    <div class="flex gap-2 py-3 overflow-x-auto scrollbar-hide">
      <button @click="loadCity('all')"
              :class="activeCity==='all' ? 'active' : ''"
      ...
      <?php foreach ($cities ?? [] as $c):
        $cName = trim($c['city'] ?? '');
        if ($cName === '') continue;
        $cSlug = strtolower($cName);
      ?>
      <button @click="loadCity('<?= esc($cSlug) ?>')"
              :class="activeCity==='<?= esc($cSlug) ?>' ? 'active' : ''"
              class="city-pill flex-shrink-0">
        <?= esc(ucfirst($cName)) ?>
      </button>
      <?php endforeach; ?>
...
<div class="max-w-6xl mx-auto px-4 pb-8">
  ...
  <div x-show="!loading" class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
    <template x-for="(st, idx) in filteredStations" :key="st.id || idx">
      <div class="station-card p-4 sm:p-5 flex flex-col" :style="`animation:fadeInUp .3s ${idx*40}ms both`">
        <!-- Header -->
        <div class="flex items-start justify-between gap-2 mb-2.5">
          <div class="flex-1 min-w-0">
            <div class="flex items-start gap-1.5">
              <h3 class="font-black text-slate-900 text-sm leading-snug line-clamp-2" x-text="st.name"></h3>
              <svg x-show="st.verified" x-cloak class="w-3.5 h-3.5 flex-shrink-0 mt-0.5" style="color:#00A896" fill="currentColor" viewBox="0 0 20 20" title="Verified">
        ...
        <!-- Address — wraps up to 2 lines instead of cutting off mid-word, show only when present -->
        <p class="text-xs text-slate-500 mb-3 flex items-start gap-1.5 min-w-0"
          ...
          <span class="line-clamp-2 leading-snug" x-text="[st.address, st.city].filter(Boolean).join(', ')"></span>
        </p>
        ...
        <!-- Connectors — colour-coded by type, underscores in raw enum values (e.g. Bharat_AC) formatted to spaces -->
        <div class="flex flex-wrap gap-1.5 mb-3" x-show="st.connector_types && st.connector_types.length">
          <template x-for="ct in (st.connector_types||[])" :key="ct">
            <span class="text-[10px] font-bold px-2 py-0.5 rounded-full" :class="connClass(ct)" x-text="fmtConn(ct)"></span>
          </template>
        </div>

        <!-- Stats -->
        <div class="flex flex-wrap gap-3 text-xs text-slate-600 mb-4">
          <span x-show="st.total_ports && st.available_ports !== null && st.available_ports !== undefined"
                class="flex items-center gap-1 font-semibold" style="color:#00963C">
            ...
            <span x-text="st.available_ports + ' of ' + st.total_ports + ' ports free'"></span>
          </span>
          <span x-show="st.total_ports && (st.available_ports === null || st.available_ports === undefined)" class="flex items-center gap-1">
        ...
        <a :href="st.lat && st.lng
           ...
           class="flex items-center justify-center gap-2 rounded-xl text-white font-bold py-2.5 text-sm transition-all duration-150 hover:-translate-y-0.5"
...
const DB_STATIONS = <?= json_encode(array_map(function($s) {
    return [
        ...
        'connector_types'=> json_decode($s['connector_types'] ?? '[]', true) ?: [],
        'total_ports'    => (int)($s['total_ports'] ?? 0),
        'available_ports'=> isset($s['available_ports']) ? (int)$s['available_ports'] : null,
        'is_open_24x7'   => !empty($s['open_24x7']),
        'price_per_kwh'  => $s['pricing_per_kwh'] ?? null,
...
// Connector-type badge colours, keyed on the normalised (space-separated) display label
const CONN_COLORS = {
  ...
  'Bharat AC': 'bg-emerald-100 text-emerald-800',
  'Bharat DC': 'bg-amber-100 text-amber-800',
};
...
function chargingApp() {
  return {
    ...
    // Raw connector enum values look like "Bharat_AC" / "Type2" — format for display and colour-code
    fmtConn(ct) { return String(ct || '').replace(/_/g, ' ').replace(/^Type(\d)$/, 'Type $1'); },
    connClass(ct) { return CONN_COLORS[this.fmtConn(ct)] || 'bg-slate-100 text-slate-700'; },
