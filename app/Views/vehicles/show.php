...
$resolveImgUrl = fn($u) => preg_match('#^https?://#i', $u) ? $u : base_url(ltrim($u, '/'));
...
@media(max-width:768px){
  ...
  /* Sticky bottom CTA bar for mobile — CarDekho-style: price left, action right */
  .mobile-sticky-cta{
    position:fixed;
    bottom:0;
    left:0;
    right:0;
    background:white;
    border-top:2px solid rgba(0,168,150,.15);
    padding:10px 14px;
  ...
  .mobile-sticky-cta .action-group{
    display:flex;
    align-items:center;
    gap:8px;
    flex-shrink:0;
  ...
  .mobile-sticky-cta .primary-btn:active{
    box-shadow:0 2px 8px rgba(0,168,150,.15);
    transform:scale(0.98);
  }

  .mobile-sticky-cta .secondary-btn{
  ...
  .mobile-sticky-cta .secondary-btn:active{
  ...
  /* Accordion styles for specs and pricing */
  ...
  details.spec-accordion > summary,
  details.pricing-accordion > summary{
    display:flex;
    align-items:center;
    justify-content:space-between;
    padding:16px;
    cursor:pointer;
    background:linear-gradient(90deg,rgba(0,230,118,.04),transparent);
  ...
  details.spec-accordion > summary:hover,
  details.pricing-accordion > summary:hover{
    background:linear-gradient(90deg,rgba(0,230,118,.08),transparent);
  }

  details.spec-accordion[open] > summary,
  details.pricing-accordion[open] > summary{
    border-bottom:2px solid rgba(0,230,118,.15);
  }

  details.spec-accordion > summary::after,
  details.pricing-accordion > summary::after{
    content:'';
    display:inline-block;
    width:18px;
    height:18px;
    background:linear-gradient(135deg,#00A896,#007A6E);
    clip-path:polygon(30% 40%, 50% 60%, 70% 40%, 85% 55%, 50% 90%, 15% 55%);
    transition:transform 0.3s ease;
    flex-shrink:0;
    margin-left:12px;
  }

  details.spec-accordion[open] > summary::after,
  details.pricing-accordion[open] > summary::after{
    transform:rotate(180deg);
  }

  /* Spec groups inside accordion */
  details.spec-accordion .spec-group{
    padding:16px;
    border-bottom:1px solid rgba(0,230,118,.08);
  }

  details.spec-accordion .spec-group:last-child{
    border-bottom:none;
  }

  details.spec-accordion .spec-group h3{
    font-size:0.875rem;
    font-weight:700;
    color:#0F172A;
    margin-bottom:12px;
  ...
  details.spec-accordion .spec-row{
    display:flex;
    justify-content:space-between;
    padding:10px 0;
    border-bottom:1px solid rgba(0,230,118,.05);
    font-size:0.875rem;
  }

  details.spec-accordion .spec-row:last-child{
    border-bottom:none;
  }

  details.spec-accordion .spec-label{
    color:#64748B;
    font-weight:500;
  }

  details.spec-accordion .spec-value{
    color:#0F172A;
    font-weight:600;
    text-align:right;
  }

  /* Hide city pricing table on mobile by default */
  #city-pricing-table{
    display:none;
  }

  #city-pricing-table[open]{
    display:block;
  }

  #city-pricing-table .pricing-content{
    padding:16px;
    max-height:400px;
    overflow-y:auto;
  }

  /* Simplified pricing card layout on mobile */
  .pricing-card{
    border:1px solid rgba(0,230,118,.12);
    border-radius:12px;
    padding:16px;
...
/* ── Extra-narrow phones (iPhone SE, budget Android ≤360px) ── */
...
<script type="application/ld+json">
{
  ...
  "image": "<?= esc($v['image_url'] ? $resolveImgUrl($v['image_url']) : '') ?>",
  "url": "<?= current_url() ?>",
...
<div class="hero-sm pt-16 sm:pt-20 md:pt-32" style="background:#F5FFF7;min-height:100vh">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
      ...
      <div>
        ...
        <div class="flex items-center flex-wrap gap-3 mb-5">
          ...
          <span class="text-sm font-semibold" style="color:#64748B"><?= $reviewCount > 0 ? $reviewCount . ' reviews' : 'No reviews yet' ?></span>
          <a href="#tab-reviews" class="text-sm font-bold hover:underline" style="color:#00A896"><?= $reviewCount > 0 ? 'Read reviews →' : 'Be the first to review →' ?></a>
        </div>
        <?php endif; ?>
...
<?php if (!empty($evVariants)): ?>
        ...
        <div class="mb-5"
             x-data="{ vars: <?= htmlspecialchars(json_encode($evVariants), ENT_QUOTES) ?>, sel: <?= $defVarIdx ?>,
                  ...
                  get minPrice(){ return Math.min(...this.vars.map(x=>x.price)); },
          ...
          <div class="flex flex-wrap gap-2 mb-3">
            <template x-for="(vv,i) in vars" :key="i">
              <button @click="sel=i" type="button"
                class="inline-flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl text-xs font-bold transition-all"
                :style="sel===i ? 'background:#00A896;color:#fff;border:1.5px solid #00A896;min-height:44px' : 'background:#fff;color:#475569;border:1.5px solid rgba(0,168,150,.22);min-height:44px'">
                <svg x-show="sel===i" x-cloak class="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                <span x-text="vv.name"></span>
                <span class="opacity-70" x-text="'· '+vv.range+' km'"></span>
                <span x-show="vv.price > minPrice" class="opacity-70" x-text="'(+'+fmt(vv.price-minPrice)+')'"></span>
              </button>
            </template>
          </div>
...
<?php endif; ?>
        ...
        <div class="flex flex-wrap gap-3">
          ...
          <a href="#lead-form" x-data @click="$dispatch('set-lead-type','book_test_ride')"
             class="inline-flex items-center gap-2 font-black text-sm px-6 py-3.5 rounded-xl transition-all"
             style="border:2px solid rgba(0,168,150,.35);color:#0F172A;background:#fff"
             onmouseover="this.style.borderColor='#00A896';this.style.background='rgba(0,168,150,.06)'"
             onmouseout="this.style.borderColor='rgba(0,168,150,.35)';this.style.background='#fff'"
          ...
          <a href="<?= base_url('compare') ?>"
             ...
             style="border:2px solid rgba(0,168,150,.2);color:#475569;background:#fff"
             onmouseover="this.style.borderColor='rgba(0,168,150,.45)';this.style.color='#0F172A'"
             onmouseout="this.style.borderColor='rgba(0,168,150,.2)';this.style.color='#475569'">
            ⚖️ Compare
          </a>
        </div>
      </div><!-- /left -->
      ...
      <?php
      // Real photos from vehicle_images table (main + gallery + interior + color), already ordered
      $galleryImgs = array_values(array_filter(array_column($vehicleImages ?? [], 'image_url')));
      if (empty($galleryImgs) && !empty($v['image_url'])) { $galleryImgs = [$v['image_url']]; }
      $galleryImgs = array_map($resolveImgUrl, $galleryImgs);
      ?>
      ...
      <div class="flex flex-col gap-2.5" x-data="{ imgs: <?= htmlspecialchars(json_encode($galleryImgs), ENT_QUOTES) ?>, active: 0, lightbox: false }">
        <!-- Main image — with slider arrows to change photo -->
        <div class="relative rounded-2xl overflow-hidden cursor-zoom-in h-60 lg:h-80" style="background:linear-gradient(145deg,#EAFFF4,#F0FFF9 55%,#F7FFFE);border:1px solid rgba(0,168,150,.16);box-shadow:0 6px 24px rgba(0,168,150,.1)" @click="lightbox=true">
          <template x-for="(im,i) in imgs" :key="i">
            <img x-show="active===i"
                 ...
                 :src="im" alt="<?= $vehicleName ?>" class="absolute inset-0 w-full h-full object-contain p-4 sm:p-6"
                 :loading="i===0 ? 'eager' : 'lazy'">
          </template>

          <!-- Prev/Next slider arrows -->
          <button type="button" @click.stop="active=(active-1+imgs.length)%imgs.length" x-show="imgs.length>1"
                  class="absolute left-2 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center transition-all"
                  style="background:rgba(255,255,255,.9);border:1px solid rgba(0,168,150,.2);box-shadow:0 2px 8px rgba(0,0,0,.1)"
                  aria-label="Previous photo">
            <svg class="w-4 h-4" style="color:#00A896" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/></svg>
          </button>
          <button type="button" @click.stop="active=(active+1)%imgs.length" x-show="imgs.length>1"
                  class="absolute right-2 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center transition-all"
                  style="background:rgba(255,255,255,.9);border:1px solid rgba(0,168,150,.2);box-shadow:0 2px 8px rgba(0,0,0,.1)"
                  aria-label="Next photo">
            <svg class="w-4 h-4" style="color:#00A896" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
          </button>

          <!-- Photo counter -->
          <div class="absolute bottom-2.5 left-2.5 flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-full pointer-events-none" style="background:rgba(15,23,42,.55);color:#fff" x-show="imgs.length>1">
            <span x-text="(active+1)+' / '+imgs.length"></span>
          </div>
        ...
        <div x-show="lightbox" x-cloak x-transition.opacity
          ...
          <template x-for="(im,i) in imgs" :key="i">
            <img x-show="active===i" @click.stop
                 ...
                 x-transition:leave-start="opacity-100"
                 x-transition:leave-end="opacity-0"
                 :src="im" class="absolute inset-0 m-auto max-h-[82vh] max-w-[90vw] object-contain rounded-xl" style="background:#fff">
          </template>
...
<!-- ── (B) VIDEO REVIEW — only rendered when a real video exists ── -->
<?php if ($ytEmbed !== ''): ?>
<section class="max-w-7xl mx-auto px-4 pt-8">
  <div class="bg-white rounded-2xl p-5 sm:p-6" style="border:1.5px solid rgba(0,230,118,.12);box-shadow:0 2px 12px rgba(0,200,100,.06)">
    <h2 class="text-xl font-black mb-4" style="color:#0F172A">Video Review</h2>
  ...
  <div class="flex flex-col lg:flex-row gap-8 items-start">
    ...
    <div class="flex-1 min-w-0">
      ...
      >
        ...
        <!-- Sticky Tab Bar -->
        <div class="sticky top-16 md:top-28 z-20 mb-6 overflow-x-auto scrollbar-hide rounded-2xl"
        ...
        <div x-show="activeTab === 'realworld'" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" role="tabpanel" aria-labelledby="tab-realworld">
          ...
          <div class="bg-white rounded-2xl p-6 mb-6" style="border:1.5px solid rgba(0,230,118,.12);box-shadow:0 2px 12px rgba(0,200,100,.06)">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <?php foreach ($rangeScenarios as $s):
                $accent = $s['pct'] >= 80 ? '#00C060' : ($s['pct'] >= 70 ? '#F59E0B' : '#EF4444');
              ?>
              <div class="rounded-2xl p-4" style="background:#F5FFF7;border:1.5px solid rgba(0,230,118,.1)">
                <div class="flex items-center gap-3 mb-3">
                  <div class="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 text-xl" style="background:<?= $accent ?>1A">
                    <?= $s['icon'] ?>
                  </div>
                  <div class="min-w-0 flex-1">
                    <p class="text-sm font-bold" style="color:#0F172A"><?= $s['label'] ?></p>
                    <p class="text-xs text-slate-500 mt-0.5"><?= $s['note'] ?></p>
                  </div>
                </div>
                <div class="flex items-end justify-between mb-2">
                  <span class="text-2xl font-black" style="color:<?= $accent ?>"><?= $s['km'] > 0 ? '~' . $s['km'] . ' km' : '—' ?></span>
                  <span class="text-xs font-bold px-2.5 py-1 rounded-full" style="background:<?= $accent ?>1A;color:<?= $accent ?>"><?= $s['pct'] ?>% of claimed</span>
                </div>
                <div class="relative h-2.5 rounded-full overflow-hidden" style="background:rgba(0,0,0,.06)" role="progressbar" aria-valuenow="<?= $s['pct'] ?>" aria-valuemin="0" aria-valuemax="100">
                  <div class="absolute left-0 top-0 h-full rounded-full transition-all duration-700" style="width:<?= $s['pct'] ?>%;background:<?= $accent ?>"></div>
                </div>
              </div>
              <?php endforeach; ?>
            </div>
        ...
        <div x-show="activeTab === 'specs'" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" role="tabpanel" aria-labelledby="tab-specs">
          ...
          <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden mb-6">
            ...
            foreach ($specGroups as $groupName => $specs):
              ...
              $groupIndex++;
              $groupParts = explode(' ', $groupName, 2);
              $groupIcon  = $groupParts[0];
              $groupLabel = $groupParts[1] ?? $groupName;
            ?>
            <div class="border-b border-slate-100 last:border-b-0" x-data="{ open: <?= $groupIndex === 1 ? 'true' : 'false' ?> }">
              <button type="button" @click="open = !open"
                      class="w-full flex items-center justify-between gap-3 px-5 py-4 transition-colors"
                      style="background:#F5FFF7" onmouseover="this.style.background='#EAFFF4'" onmouseout="this.style.background='#F5FFF7'"
                      :aria-expanded="open.toString()">
                <div class="flex items-center gap-3">
                  <div class="w-9 h-9 rounded-xl flex items-center justify-center text-lg flex-shrink-0" style="background:rgba(0,230,118,.14)"><?= $groupIcon ?></div>
                  <h3 class="text-sm font-black uppercase tracking-wide" style="color:#0F172A"><?= esc($groupLabel) ?></h3>
                </div>
                <svg class="w-4 h-4 flex-shrink-0 transition-transform duration-200" :class="open ? 'rotate-180' : ''" style="color:#00A896" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/></svg>
              </button>
              <div x-show="open" x-collapse>
                <div class="grid sm:grid-cols-2 gap-2.5 px-5 pb-5 pt-1">
                  <?php foreach ($specs as [$label, $value]):
                    if (empty($value)) continue;
                  ?>
                  <div class="flex items-center justify-between gap-3 px-4 py-3 rounded-xl transition-colors" style="background:#FAFFFE;border:1px solid rgba(0,230,118,.08)" onmouseover="this.style.background='#F0FFF9'" onmouseout="this.style.background='#FAFFFE'">
                    <span class="text-xs font-semibold" style="color:#64748B"><?= $label ?></span>
                    <span class="text-sm font-black text-right" style="color:#0F172A"><?= $value ?></span>
                  </div>
                  <?php endforeach; ?>
                </div>
              </div>
        ...
        <div x-show="activeTab === 'charging'" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" role="tabpanel" aria-labelledby="tab-charging">
          ...
          <div class="bg-white rounded-2xl p-6 mb-6" style="border:1.5px solid rgba(0,230,118,.12);box-shadow:0 2px 12px rgba(0,200,100,.06)">
            ...
            <div class="flex items-center gap-3 mb-5 p-4 rounded-2xl" style="background:#F5FFF7;border:1.5px solid rgba(0,230,118,.1)">
              <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 text-2xl" style="background:rgba(0,168,150,.12)">🔌</div>
              <div class="min-w-0">
                <p class="text-xs font-black uppercase tracking-wide" style="color:#94A3B8">Connector Type</p>
                <div class="flex items-center gap-2 mt-1.5 flex-wrap">
                  ...
                  <span class="text-xs font-bold px-3 py-1 rounded-full" style="background:rgba(245,158,11,.12);color:#B45309;border:1px solid rgba(245,158,11,.25)">⚡ Fast Charging Supported</span>
                  <?php endif; ?>
                </div>
              </div>
            </div>
            ...
            <!-- Compatible networks -->
            <div class="mb-5">
              <h3 class="text-sm font-black uppercase tracking-wide mb-3" style="color:#0F172A">Compatible Charging Networks</h3>
              <div class="flex flex-wrap gap-2">
                <?php foreach ($chargingNetworks as $network): ?>
                <span class="inline-flex items-center gap-1.5 text-sm font-bold px-3.5 py-1.5 rounded-full" style="background:rgba(0,230,118,.08);color:#007A6E;border:1px solid rgba(0,230,118,.22)">
            ...
            <div class="rounded-2xl p-5 flex items-start gap-4" style="background:rgba(0,230,118,.06);border:1.5px solid rgba(0,230,118,.18)">
              <div class="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 text-xl" style="background:rgba(0,230,118,.14)">🏠</div>
              <div>
                <h3 class="text-sm font-black uppercase tracking-wide mb-1.5" style="color:#166534">Home Charger Recommendation</h3>
                <p class="text-sm leading-relaxed" style="color:#334155">
                ...
                <a href="<?= base_url('home-charger-guide') ?>" class="inline-flex items-center gap-1 font-black text-sm mt-2" style="color:#00A896">
        ...
        <div x-show="activeTab === 'reviews'" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" role="tabpanel" aria-labelledby="tab-reviews" id="tab-reviews">
          ...
          <div class="bg-white rounded-2xl p-6 mb-6" style="border:1.5px solid rgba(0,230,118,.12);box-shadow:0 2px 12px rgba(0,200,100,.06)">
            ...
            <?php else: ?>
            <!-- No reviews yet — premium empty state, matches the dashed-card pattern used elsewhere -->
            <div class="rounded-2xl flex flex-col items-center text-center gap-3 py-14 px-6"
              ...
              <span class="w-14 h-14 rounded-full flex items-center justify-center" style="background:rgba(0,168,150,.12)">
                <svg class="w-7 h-7" style="color:#00A896" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
              </span>
              <h3 class="text-lg font-black" style="color:#0F172A">Be the first to review the <?= esc($v['name'] ?? '') ?></h3>
              <p class="text-sm font-semibold max-w-sm" style="color:#64748B">Share real range, charging experience &amp; ownership costs — help other buyers decide with confidence.</p>
              <a
                href="<?= base_url('reviews/submit/' . esc($v['slug'] ?? '')) ?>"
                class="inline-flex items-center gap-2 text-white font-black px-6 py-3 rounded-xl transition-all mt-1"
                style="background:linear-gradient(135deg,#00C060,#009944)"
                onmouseover="this.style.boxShadow='0 4px 14px rgba(0,200,100,.4)'"
                onmouseout="this.style.boxShadow=''"
              >
        ...
        <!-- ── TAB: OWNER Q&A ────────────────────────────── -->
        <div id="owner-qa-section" x-show="activeTab === 'ownerqa'" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" role="tabpanel" aria-labelledby="tab-ownerqa">
        ...
        <div class="flex gap-4 overflow-x-auto pb-3 -mx-1 px-1 snap-x snap-mandatory scroll-smooth">
          ...
          >
            <div class="h-36 bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center relative overflow-hidden">
              ...
              <img src="<?= esc($resolveImgUrl($sv['image_url'])) ?>" alt="<?= esc($sv['name'] ?? '') ?>" class="h-full w-full object-cover group-hover:scale-105 transition-transform duration-300" loading="lazy">
              <?php else: ?>
    ...
    <!-- ── MOBILE STICKY CTA BAR (hidden on desktop) — price left, action right (CarDekho style) ── -->
    <div class="mobile-sticky-cta md:hidden" x-data="{ modal: null }">
      <div class="price-block">
        <p class="price-label">Starting from</p>
        <p class="price-value"><?= fmtINRShort($startingPrice ?: null) ?></p>
      </div>
      <div class="action-group">
        <button type="button" class="secondary-btn" @click="modal = 'price_alert'"
                aria-label="Price Drop Alert"
                onclick="charjTrack('mobile_price_alert_open',{vehicle:'<?= esc(addslashes($vehicleName)) ?>'})">
          🔔
        </button>
        <button type="button" class="primary-btn" @click="modal = 'test_ride'"
                onclick="charjTrack('mobile_test_ride_open',{vehicle:'<?= esc(addslashes($vehicleName)) ?>'})">
          Book Test Ride
        </button>
      </div>

      <!-- Mobile Test Ride Modal (reuse from sidebar) -->
      ...
      <!-- Mobile Price Alert Modal (reuse from sidebar) -->
      <div x-show="modal === 'price_alert'" x-cloak class="fixed inset-0 z-[200] flex items-center justify-center p-4"
        ...
        <div class="relative w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto"
          ...
          <div class="flex items-center justify-between gap-3 px-5 py-4" style="background:linear-gradient(135deg,#F0FFF9,#EAFFF4);border-bottom:1px solid rgba(0,168,150,.12)">
            <div>
              <h3 class="font-black text-base" style="color:#0F172A">🔔 Price Drop Alert</h3>
              <p class="text-xs font-semibold" style="color:#64748B"><?= $vehicleName ?></p>
            </div>
            <button type="button" @click="modal = null" aria-label="Close" class="w-8 h-8 rounded-full flex items-center justify-center transition-colors" style="color:#64748B" onmouseover="this.style.background='rgba(0,0,0,.06)'" onmouseout="this.style.background=''">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
          ...
          <form action="<?= base_url('lead/submit') ?>" method="post" class="p-5 space-y-3.5">
            ...
            <p class="text-[11px] text-center text-slate-400">We'll notify you about price changes and launches.</p>
          </form>
        </div>
      </div>
    </div>
    ...
    <aside class="w-full lg:w-80 xl:w-96 flex-shrink-0">
      <div class="sticky top-20 md:top-32 space-y-4">
