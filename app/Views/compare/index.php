...
function cmpRow(string $label, array $vv, string $field, string $mode = 'text', string $unit = '', bool $formatPrice = false, string $varKey = '', array $colVariants = []): string {
    ...
    foreach ($vals as $i => $raw) {
        ...
        $valAttr  = $reactive
            ...
            : '';
        $html .= '<div class="px-4 py-3.5 text-sm text-center font-medium" style="border-left:1px solid rgba(0,230,118,.07)' . ($isW ? ';background:rgba(0,230,118,.08)' : '') . '">';
        $html .= $isW
...
/* Comparison table */
.cmp-table{overflow-x:auto;-webkit-overflow-scrolling:touch;scroll-snap-type:x proximity}
.cmp-row{display:grid;border-bottom:1px solid rgba(0,230,118,.07);opacity:0;transform:translateX(-10px);transition:opacity .35s ease,transform .35s ease}
.cmp-row.visible{opacity:1;transform:none}
...
@media(max-width:767px){
  ...
  /* ── Grid override: 110px label + auto vehicle columns (min 120px each) ── */
  /* !important overrides PHP-generated inline grid-template-columns */
  .cmp-row,.cmp-section-label,
  #compare-table .grid{
    grid-template-columns:110px repeat(auto-fill,minmax(120px,1fr))!important;
  }
  /* Min-width: 110px label + 3×120px vehicles = 470px — forces scroll on 375px phones */
  .cmp-table>*,.cmp-row,.cmp-section-label{min-width:470px!important}
...
/* Sticky bar */
#cmp-sticky{position:fixed;top:0;left:0;right:0;z-index:50;background:linear-gradient(160deg,#F8FBFF,#EEF6FF);backdrop-filter:blur(16px);border-bottom:2px solid #00E676;box-shadow:0 4px 20px rgba(0,0,0,.06);transform:translateY(-100%);transition:transform .3s ease}
...
.sec-reveal{opacity:0;transform:translateX(-16px);transition:opacity .4s ease,transform .4s cubic-bezier(.22,1,.36,1)}
.sec-reveal.visible{opacity:1;transform:none}
  ...
  <div class="max-w-7xl mx-auto px-4 py-3 flex items-center gap-4 overflow-x-auto">
    ...
    <div class="flex items-center gap-2 flex-shrink-0">
      <div class="w-8 h-8 rounded-lg flex items-center justify-center overflow-hidden flex-shrink-0" style="background:rgba(0,230,118,.08);border:1px solid rgba(0,230,118,.2)">
        ...
        <img src="<?= esc($si) ?>" class="w-full h-full object-contain" loading="lazy">
...
<!-- HERO HEADER -->
<div class="relative overflow-hidden pt-16 pb-6 px-4 sm:px-6" style="background:#F7FFFE">
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-6 cmp-page-bg" x-data="compareApp()" x-init="init()">
  ...
  <div class="rounded-2xl overflow-hidden" style="background:#FFFFFF;border:1px solid rgba(0,168,150,.2);box-shadow:0 8px 48px rgba(0,168,150,.12),0 2px 12px rgba(0,0,0,.06)">
    ...
    <div class="p-6">
      ...
      <div class="flex flex-col sm:flex-row items-stretch gap-0 sm:gap-0 mb-6">
        ...
        <!-- SLOT -->
        <div x-data="slotSearch(<?= $slot ?>, <?= $svJson ?>)" class="relative flex-1 anim-slot-<?= $slot ?>">
  ...
  <!-- ═══ COMPARISON TABLE ═══ -->
  <div id="compare-table" class="rounded-2xl overflow-hidden"
    ...
    <div class="cmp-table">
      ...
      <div class="grid" style="grid-template-columns:200px repeat(<?= $count ?>,1fr);border-bottom:2px solid rgba(0,168,150,.1)">
        ...
        <div class="p-5 flex items-end" style="background:linear-gradient(135deg,rgba(0,168,150,.08),rgba(0,168,150,.03));border-right:1px solid rgba(0,168,150,.12)">
          <span class="text-[10px] font-bold uppercase tracking-widest" style="color:#00A896">Specification</span>
        </div>
        <?php foreach ($vehicles as $vi => $vv):
          $vImg = $vv['featured_image'] ?? $vv['image_url'] ?? '';
          $isTop = ($vi === $bestIdx);
        ?>
        <div class="flex flex-col items-center text-center relative anim-veh-<?= $vi ?>" style="border-left:1px solid rgba(0,168,150,.08);<?= $isTop ? 'background:linear-gradient(180deg,rgba(0,168,150,.06) 0%,#fff 60%)' : 'background:#fff' ?>">
          <?php if ($isTop): ?>
          ...
          <?php endif; ?>
          <!-- EV icon placeholder (no broken image) -->
  ...
  <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
    ...
    <div class="rounded-2xl p-5 transition-all anim-bf-<?= $bfi ?>" style="background:#FFFFFF;border:1px solid rgba(0,168,150,.14);box-shadow:0 2px 10px rgba(0,0,0,.05);border-top:3px solid rgba(0,168,150,.25)"
      ...
      <div class="flex items-center gap-3 mb-3">
        <?php if ($bImg): ?><img src="<?= esc($bImg) ?>" alt="" class="h-10 w-14 object-contain rounded" loading="lazy"><?php endif; ?>
        <div>
