...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    /* Heading weight hierarchy, enforced site-wide. Individual pages have accumulated
       inconsistent font-bold/font-semibold classes on headings over time; Tailwind's
       utility classes have equal-or-higher specificity than a plain element selector,
       so this needs !important to actually win and keep h1–h4 weight consistent everywhere. */
    ...
    /* Touch press feedback — touch-only devices (real mouse hover untouched) */
    @media (hover: none) {
      .card:active,.card-hover:active,.vc-card:active,.panel-card:active,.cat-card:active,.brand-dir-card:active {
        transform: scale(.97);
        transition: transform .1s ease;
      }
      .station-row:active { transform: scale(.98); transition: transform .1s ease; }
    }
    ...
    .nav-link:hover { color:#00A896; background:rgba(0,168,150,.08); }
...
<body class="antialiased" style="background:#F5FFF7;color:#0F172A" x-data="{mobileOpen:false,searchOpen:false}"
      @keydown.cmd.k.window.prevent="searchOpen=true;$nextTick(()=>$refs.globalSearchInput?.focus())"
      @keydown.ctrl.k.window.prevent="searchOpen=true;$nextTick(()=>$refs.globalSearchInput?.focus())">
...
<header class="fixed inset-x-0 z-[200]"
        style="top:<?= session()->get('admin_previewing_as_customer') ? '40px' : '0' ?>;background:rgba(255,255,255,.96);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border-bottom:1px solid rgba(0,168,150,.1);box-shadow:0 1px 0 rgba(0,0,0,.04),0 4px 24px rgba(0,0,0,.06);transform:translateZ(0);will-change:transform;contain:layout style">
  <div class="mx-auto max-w-7xl px-4 sm:px-6">

    <!-- ═══ ROW 1: Logo + centered search bar + account icons (CarDekho-style) ═══ -->
    <div class="flex h-[66px] items-center gap-3">
      ...
      <!-- Search bar — always visible, centered, real GET search to /vehicles -->
      <div class="relative flex-1 max-w-2xl mx-auto" @keydown.escape.window="searchOpen=false" @click.outside="searchOpen=false">
        <form method="GET" action="<?= base_url('vehicles') ?>"
              class="flex items-center gap-2 w-full rounded-full pl-4 pr-1.5 py-1.5 transition-all"
              style="border:1.5px solid rgba(0,168,150,.22);background:#FAFFFE">
          <span class="hidden sm:inline-block pr-3 flex-shrink-0 font-semibold text-sm whitespace-nowrap"
                style="color:#0F172A;border-right:1.5px solid rgba(0,168,150,.15)">
            All EVs
          </span>
          <svg class="w-4 h-4 flex-shrink-0" style="color:#94A3B8" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-4.35-4.35M17 11A6 6 0 1 1 5 11a6 6 0 0 1 12 0z"/>
          </svg>
          <input type="text" name="q" x-ref="globalSearchInput" autocomplete="off"
                 @focus="searchOpen=true"
                 placeholder="Search EVs, brands & more…"
                 class="flex-1 min-w-0 outline-none text-sm bg-transparent py-1.5" style="color:#0F172A">
          <button type="submit"
                  class="hidden sm:flex items-center justify-center w-8 h-8 rounded-full flex-shrink-0 transition-all"
                  style="background:linear-gradient(135deg,#00A896,#00C9B1);color:#fff" aria-label="Search">
        ...
        <!-- Dropdown panel — anchored directly below the search bar -->
        <div x-show="searchOpen" x-cloak
             ...
             class="absolute left-0 right-0 top-full mt-2 rounded-2xl shadow-2xl overflow-hidden max-h-[70vh] flex flex-col z-10"
             style="background:#fff;border:1px solid rgba(0,168,150,.12)">
          <div class="overflow-y-auto p-5">

            <!-- Explore shortcuts — real filtered links, not generative AI -->
            <p class="text-[11px] font-black uppercase tracking-wider mb-3" style="color:#94A3B8">Explore</p>
            <div class="flex flex-col gap-2 mb-6">
              <a href="<?= base_url('vehicles') ?>?price_max=1500000&sort=rating_desc" @click="searchOpen=false"
                 class="inline-flex items-center gap-2 px-4 py-2.5 rounded-full text-sm font-semibold transition-colors"
                 style="border:1.5px solid rgba(0,168,150,.25);color:#0F172A"
                 onmouseover="this.style.background='rgba(0,168,150,.06)'" onmouseout="this.style.background='transparent'">
                <span style="color:#00A896">✦</span> Premium EVs under ₹15 Lakh
              </a>
              <a href="<?= base_url('vehicles') ?>?sort=range_desc" @click="searchOpen=false"
                 class="inline-flex items-center gap-2 px-4 py-2.5 rounded-full text-sm font-semibold transition-colors"
                 style="border:1.5px solid rgba(0,168,150,.25);color:#0F172A"
                 onmouseover="this.style.background='rgba(0,168,150,.06)'" onmouseout="this.style.background='transparent'">
                <span style="color:#00A896">✦</span> Longest range EVs
              </a>
              <a href="<?= base_url('vehicles') ?>?price_max=500000&sort=price_asc" @click="searchOpen=false"
                 class="inline-flex items-center gap-2 px-4 py-2.5 rounded-full text-sm font-semibold transition-colors"
                 style="border:1.5px solid rgba(0,168,150,.25);color:#0F172A"
                 onmouseover="this.style.background='rgba(0,168,150,.06)'" onmouseout="this.style.background='transparent'">
                <span style="color:#00A896">✦</span> Budget EVs under ₹5 Lakh
              </a>
              <a href="<?= base_url('vehicles') ?>?sort=newest" @click="searchOpen=false"
                 class="inline-flex items-center gap-2 px-4 py-2.5 rounded-full text-sm font-semibold transition-colors"
                 style="border:1.5px solid rgba(0,168,150,.25);color:#0F172A"
                 onmouseover="this.style.background='rgba(0,168,150,.06)'" onmouseout="this.style.background='transparent'">
                <span style="color:#00A896">✦</span> Newest EV launches
              </a>
            </div>

            <!-- Popular Searches -->
            <p class="text-[11px] font-black uppercase tracking-wider mb-1" style="color:#94A3B8">Popular Searches</p>
            <div class="flex flex-col">
              <?php
              $popularSearches = [
                ['label' => 'All Vehicles',   'href' => base_url('vehicles')],
                ['label' => 'Tata Nexon EV',  'href' => base_url('vehicles') . '?q=Tata+Nexon+EV'],
                ['label' => 'Ather 450X',     'href' => base_url('vehicles') . '?q=Ather+450X'],
                ['label' => 'MG ZS EV',       'href' => base_url('vehicles') . '?q=MG+ZS+EV'],
                ['label' => 'Ola S1 Pro',     'href' => base_url('vehicles') . '?q=Ola+S1+Pro'],
              ];
              foreach ($popularSearches as $i => $ps):
              ?>
              <a href="<?= $ps['href'] ?>" @click="searchOpen=false"
                 class="flex items-center gap-2.5 py-3 text-sm font-medium transition-colors"
                 style="color:#334155;<?= $i > 0 ? 'border-top:1px solid rgba(0,168,150,.08)' : '' ?>"
                 onmouseover="this.style.color='#00A896'" onmouseout="this.style.color='#334155'">
    ...
    <!-- ═══ ROW 2: Nav links (left) + Find My EV CTA (right) — desktop only ═══ -->
    <?php $curUri = current_url(true)->getPath(); ?>
    <div class="hidden md:grid h-[46px] items-center gap-4" style="grid-template-columns:1fr auto 1fr;border-top:1px solid rgba(0,168,150,.08)">
      <div></div>
      <nav class="flex items-center gap-0.5 justify-self-center" aria-label="Main navigation">
      ...
      <div class="justify-self-end">
        <a href="<?= base_url('find-my-ev') ?>" onclick="charjTrack('header_cta_click',{location:'header'})"
           class="inline-flex items-center gap-2 font-bold text-sm px-5 py-2 rounded-full transition-all duration-200 ripple-btn flex-shrink-0"
...
<!-- Backdrop for the search dropdown — dims page content below the header, header itself stays fully visible -->
<div x-show="searchOpen" x-cloak class="fixed inset-0 z-[190]" style="background:rgba(15,23,42,.4);backdrop-filter:blur(2px)" @click="searchOpen=false"></div>
...
.ft-acc summary{cursor:pointer;list-style:none;display:flex;align-items:center;justify-content:space-between}
.ft-acc summary::-webkit-details-marker{display:none}
.ft-acc summary::after{content:'+';color:#94A3B8;font-weight:400;font-size:.8rem}
.ft-acc[open] summary::after{content:'\2212'}
@media (min-width:640px){
  .ft-acc, .ft-acc > div{display:block!important}
  .ft-acc summary{cursor:default;pointer-events:none}
...
<footer class="pb-20 md:pb-0" style="background:#F0FFF8;border-top:1px solid rgba(0,168,150,.12)" role="contentinfo">
  <div class="max-w-6xl mx-auto px-4 sm:px-6 py-5">
    <div class="flex flex-col sm:flex-row sm:items-start gap-5">
      ...
      <!-- 4 link columns (accordion on mobile) -->
      <div class="flex-1 grid grid-cols-1 sm:grid-cols-4 gap-0 sm:gap-4">

        <details class="ft-acc py-2 sm:py-0" style="border-bottom:1px solid rgba(0,168,150,.1)">
          <summary class="text-[9px] font-black uppercase tracking-widest mb-2 py-1 sm:py-0" style="color:#00A896">Explore</summary>
        ...
        <details class="ft-acc py-2 sm:py-0" style="border-bottom:1px solid rgba(0,168,150,.1)">
          <summary class="text-[9px] font-black uppercase tracking-widest mb-2 py-1 sm:py-0" style="color:#00A896">Tools</summary>
          <div class="pb-1 sm:pb-0">
        ...
        <details class="ft-acc py-2 sm:py-0" style="border-bottom:1px solid rgba(0,168,150,.1)">
          <summary class="text-[9px] font-black uppercase tracking-widest mb-2 py-1 sm:py-0" style="color:#00A896">Resources</summary>
          <div class="pb-1 sm:pb-0">
        ...
        <details class="ft-acc py-2 sm:py-0">
          <summary class="text-[9px] font-black uppercase tracking-widest mb-2 py-1 sm:py-0" style="color:#00A896">Company</summary>
          <div class="pb-1 sm:pb-0">
...
<!-- Recently Viewed EVs — localStorage widget (compact FAB, expands on tap) -->
<div id="rv-widget"
     x-data="recentlyViewedWidget()"
     x-init="init()"
     x-show="items.length > 0"
     x-cloak
     class="fixed bottom-24 right-4 z-30 md:bottom-6 md:right-6 flex flex-col items-end">
  <!-- Expanded panel -->
  <div x-show="open" x-cloak x-transition
       @click.outside="open=false"
       class="mb-2 w-56 rounded-2xl overflow-hidden"
       style="background:#FFFFFF;border:1.5px solid rgba(0,168,150,.2);filter:drop-shadow(0 8px 24px rgba(0,168,150,.18))">
    <div class="flex items-center justify-between px-3 py-2" style="background:rgba(0,168,150,.07);border-bottom:1px solid rgba(0,168,150,.12)">
      ...
      <button @click="open=false" class="text-xs font-bold px-1.5 rounded" style="color:#94A3B8">✕</button>
    </div>
    <div class="py-1.5 max-h-48 overflow-y-auto scrollbar-hide">
  ...
  <button @click="open=!open" x-show="!open"
          ...
          style="background:#FFFFFF;border:1.5px solid rgba(0,168,150,.25);filter:drop-shadow(0 6px 16px rgba(0,168,150,.2))"
          aria-label="Recently viewed EVs">
    <span class="text-lg">🕘</span>
    <span class="absolute -top-1 -right-1 min-w-[16px] h-4 px-1 rounded-full text-[9px] font-black text-white flex items-center justify-center"
          style="background:#00A896" x-text="items.length"></span>
  </button>
</div>
...
<!-- ── Floating Compare Bar — only on pages where comparing vehicles is relevant ── -->
<?php
$compareBarUri     = trim(current_url(true)->getPath(), '/');
$compareBarAllowed = ($compareBarUri === '') // homepage
    || preg_match('#^(vehicles|explore|brands|compare)(/|$)#i', $compareBarUri);
?>
<?php if ($compareBarAllowed): ?>
<div id="charj-compare-bar"
  ...
  <div class="max-w-5xl mx-auto px-3 sm:px-4 py-2.5 sm:py-3">

    <!-- Row 1: label (desktop) + chips (always) + actions (desktop only) -->
    <div class="flex items-center gap-2 sm:gap-3">

      <!-- Label — desktop only, mobile has no room for it -->
      <span class="hidden sm:inline text-xs font-black uppercase tracking-widest flex-shrink-0" style="color:#00E676">Compare</span>

      <!-- Selected vehicles -->
      <div class="flex-1 flex items-center gap-1.5 sm:gap-2 overflow-x-auto scrollbar-hide min-w-0">
        <template x-for="item in items" :key="item.slug">
          <div class="flex items-center gap-1 sm:gap-1.5 rounded-lg sm:rounded-xl px-2 sm:px-3 py-1 sm:py-1.5 flex-shrink-0 transition-all duration-200"
               style="background:rgba(0,230,118,.08);border:1px solid rgba(0,230,118,.3)">
            <span class="text-white text-[11px] sm:text-xs font-bold truncate max-w-[86px] sm:max-w-[120px]" x-text="item.name"></span>
        ...
        <template x-if="count < 2">
          <a href="<?= base_url('vehicles') ?>"
             class="flex items-center gap-1 sm:gap-1.5 rounded-lg sm:rounded-xl px-2 sm:px-3 py-1 sm:py-1.5 flex-shrink-0 transition-all duration-200"
            ...
            <span class="text-[10px] sm:text-[11px] font-bold whitespace-nowrap" style="color:#00E676">add one more</span>
          </a>
        </template>
      </div>

      <!-- Actions — desktop only (mobile gets a full-width row 2 below) -->
    ...
    <!-- Row 2: mobile-only full-width CTA + clear -->
    <div class="flex sm:hidden items-center gap-2 mt-2">
      <a :href="compareUrl()"
         class="flex-1 flex items-center justify-center gap-1.5 font-black text-xs py-2 rounded-xl transition-all duration-200"
         :style="count >= 2
           ? 'background:linear-gradient(135deg,#00E676,#00C060);color:#022C22'
           : 'background:rgba(0,230,118,.15);color:#00E676;border:1px solid rgba(0,230,118,.3)'">
        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>
        <span x-text="count >= 2 ? 'Compare Now' : 'Browse EVs'"></span>
      </a>
      <button type="button" @click="clear()"
              class="w-9 h-9 flex-shrink-0 rounded-xl flex items-center justify-center transition-all duration-200"
              style="background:rgba(0,230,118,.08);color:#6EE7A8"
              title="Clear all">
        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
      </button>
