...
<!-- Hero -->
<section class="hero-sm pt-24 pb-8 px-4 relative overflow-hidden" style="background:linear-gradient(135deg,#F0FFF9,#EAFFF4,#F7FFFE)">
    ...
    <div class="lg:grid lg:grid-cols-2 lg:gap-8 lg:items-start">
      ...
      <div class="mt-6 lg:mt-0 space-y-5">
        ...
        <div class="rounded-3xl shadow-xl p-6 sm:p-8 text-white" style="background:linear-gradient(150deg,#00A896,#007A6E)">
          <template x-if="stops === 0">
            <div class="text-center">
              <div class="text-5xl mb-2">✓</div>
              <div class="text-2xl sm:text-3xl font-black">Doable in one charge</div>
              <p class="text-white/80 text-sm mt-2" x-text="'Your ~' + usableFirst + ' km of usable range covers the full ' + distance + ' km trip.'"></p>
            </div>
          </template>
          <template x-if="stops > 0">
            <div class="text-center">
              <div class="text-5xl sm:text-6xl font-black leading-none" x-text="stops"></div>
              <div class="text-lg sm:text-xl font-bold mt-1" x-text="stops === 1 ? 'charging stop needed' : 'charging stops needed'"></div>
              <div class="inline-flex items-center gap-2 mt-4 rounded-full px-4 py-1.5 text-sm font-semibold" style="background:rgba(255,255,255,.15)">
                ⏱ <span x-text="'~' + chargeTime + ' min total charging'"></span>
              </div>
        ...
        <div class="bg-white rounded-3xl shadow-xl p-6 sm:p-8" style="border:1px solid #EAFFF4">
          ...
          <div class="relative pl-8">
            ...
            <div class="relative mb-6">
              ...
              <div class="text-sm font-bold" style="color:#0F172A" x-text="startLabel"></div>
              <div class="text-xs" style="color:#64748B" x-text="'0 km · start at ' + startCharge + '%'"></div>
            </div>

            <!-- Charging stops -->
            <template x-for="(km,idx) in stopMarks" :key="idx">
              <div class="relative mb-6">
            ...
            <div class="relative">
              ...
              <div class="text-sm font-bold" style="color:#0F172A" x-text="destLabel"></div>
              <div class="text-xs" style="color:#64748B" x-text="distance + ' km · destination'"></div>
            </div>
          </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <a href="<?= base_url('can-i-make-it') ?>" class="rounded-2xl p-4 text-center transition-all hover:shadow-md" style="background:#EAFFF4;border:1px solid #d7f5ec">
          ...
          <a href="<?= base_url('charging-stations') ?>" class="rounded-2xl p-4 text-center transition-all hover:shadow-md" style="background:#EAFFF4;border:1px solid #d7f5ec">
          ...
          <a href="<?= base_url('charging-cost') ?>" class="rounded-2xl p-4 text-center transition-all hover:shadow-md" style="background:#EAFFF4;border:1px solid #d7f5ec">
            <div class="text-xl mb-1">💰</div>
...
function routePlanner() {
  return {
    ...
    onRouteChange() {
      // no-op; reactive getters handle the rest
    },
    ...
    get startLabel() {
      return this.routeKey === 'custom' ? 'Start' : this.routes[this.routeKey].from;
    },
    get destLabel() {
      return this.routeKey === 'custom' ? 'Destination' : this.routes[this.routeKey].to;
    },

    // Usable range on the first leg (depart at startCharge, never drop below buffer)
    get usableFirst() {
      return Math.round(this.realRange * ((this.startCharge - this.minBuffer) / 100));
    },
    // Usable range on subsequent legs (fast charge tapers, assume ~80%)
    get usableNext() {
      return Math.round(this.realRange * ((80 - this.minBuffer) / 100));
    },

    get stops() {
      if (this.distance <= this.usableFirst) return 0;
      let remaining = this.distance - this.usableFirst;
      // each charge stop adds usableNext of reach
      let per = Math.max(1, this.usableNext); // guard against div by zero
      return 1 + Math.floor(Math.max(0, remaining - 0.000001) / per);
    },

    get stopMarks() {
      let marks = [];
      if (this.stops === 0) return marks;
      let pos = this.usableFirst; // distance covered when first stop is reached
      for (let i = 0; i < this.stops; i++) {
        marks.push(Math.min(this.distance, Math.round(pos)));
        pos += this.usableNext;
      }
      return marks;
    },
