...
.tool-hero{background:linear-gradient(135deg,#F0FFF9,#EAFFF4,#F7FFFE);}
.compat-row{border-radius:.875rem;padding:1rem 1.25rem;transition:all .2s;border:1.5px solid #EAFFF4;background:#fff;}
.compat-row:hover{border-color:#00A896;background:#F0FFF9;}
.compat-yes{color:#007A6E;font-weight:800;font-size:.75rem;display:flex;align-items:center;gap:.3rem;}
...
.time-chip{font-size:.65rem;color:#475569;background:#F5FFF7;border:1px solid #d7f5ec;padding:.2rem .6rem;border-radius:9999px;margin-top:.2rem;}
</style>
<?= $this->endSection() ?>
...
}" style="background:#F7FFFE" class="min-h-screen">

<!-- Hero -->
<section class="hero-sm tool-hero pt-20 sm:pt-24 md:pt-32 pb-5 sm:pb-8 px-4 relative overflow-hidden">
  <div class="relative max-w-3xl mx-auto text-center">
    <div class="inline-flex items-center gap-1.5 rounded-full px-3 py-1 sm:px-4 sm:py-1.5 text-[10px] sm:text-xs font-bold uppercase tracking-widest mb-2.5 sm:mb-5"
         style="background:rgba(0,168,150,.10);border:1px solid rgba(0,168,150,.25);color:#00A896">
      🔌 Charger Compatibility Checker
    </div>
    <h1 class="text-2xl sm:text-5xl font-black leading-tight mb-1.5 sm:mb-3" style="color:#0F172A">
      Is Your EV Compatible with Public Chargers?
    </h1>
    <p class="hidden sm:block text-base max-w-xl mx-auto" style="color:#475569">
...
<!-- Tool Content -->
<div class="max-w-5xl mx-auto px-4 pt-4 sm:pt-6 pb-16 lg:grid lg:grid-cols-[380px_1fr] lg:gap-8 lg:items-start">

  <!-- EV Selector Card -->
  <div class="bg-white rounded-3xl shadow-xl p-4 sm:p-6 mb-4 sm:mb-5 lg:mb-0 lg:sticky lg:top-24" style="border:1px solid #EAFFF4;animation:fadeInUp .4s ease both">
    <label class="block text-sm font-black mb-2.5 sm:mb-3" style="color:#0F172A">Select your EV</label>
    <div class="relative">
      <select x-model="selected"
              class="w-full rounded-2xl px-4 py-3.5 font-semibold text-sm focus:outline-none focus:ring-2 appearance-none bg-white transition-all cursor-pointer"
              style="border:2px solid #d7f5ec;color:#0F172A;--tw-ring-color:rgba(0,168,150,.15)">
      ...
      <div class="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none">
        <svg class="w-5 h-5" style="color:#94A3B8" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path d="M19 9l-7 7-7-7"/></svg>
      </div>
    </div>
    <!-- Connector tags -->
    <div x-show="ev" x-transition class="mt-4 flex flex-wrap gap-2">
      <span class="text-xs font-bold self-center" style="color:#64748B">Connectors:</span>
      <template x-for="c in ev?.connectors" :key="c">
        <span class="text-xs font-semibold text-white px-3 py-1.5 rounded-full" style="background:#007A6E" x-text="c"></span>
      </template>
    </div>
  </div>
  ...
  <div x-show="ev" x-cloak x-transition class="space-y-3 mb-5" style="animation:fadeInUp .3s ease both">
    <h2 class="text-sm font-black uppercase tracking-wide px-1" style="color:#0F172A">Charger Compatibility</h2>
    ...
    <div class="compat-row flex items-center justify-between gap-3">
      <div class="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div class="min-w-0">
          <div class="text-sm font-bold truncate" style="color:#0F172A"><?= $label ?></div>
          <div class="text-xs" style="color:#94A3B8"><?= $sub ?></div>
        </div>
      </div>
    ...
    <!-- Notes -->
    <div x-show="ev?.notes" class="rounded-2xl p-4 flex items-start gap-3" style="background:rgba(0,168,150,.06);border:1px solid rgba(0,168,150,.15)">
      <svg class="w-4 h-4 flex-shrink-0 mt-0.5" style="color:#00A896" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
      <p class="text-xs font-medium leading-relaxed" style="color:#007A6E" x-text="ev?.notes"></p>
    </div>
  </div>

  <!-- Empty state -->
  <div x-show="!ev" class="bg-white rounded-3xl p-8 sm:p-12 lg:p-16 text-center shadow-sm" style="border:1px dashed #d7f5ec;animation:fadeInUp .5s ease both">
    <div class="w-12 h-12 sm:w-14 sm:h-14 mx-auto mb-3 sm:mb-4 rounded-2xl flex items-center justify-center text-xl sm:text-2xl" style="background:rgba(0,168,150,.1)">🔌</div>
    <h3 class="text-base sm:text-lg font-black mb-1.5 sm:mb-2" style="color:#0F172A">Select your EV above</h3>
    <p class="text-sm" style="color:#94A3B8">We'll show exactly which chargers work with it</p>
  </div>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-5">
    ...
    <a href="<?= base_url('home-charger-guide') ?>"
       class="flex items-center justify-center gap-2 rounded-2xl text-white font-bold py-3.5 text-sm transition-all hover:-translate-y-0.5"
       style="background:#0F172A">
      🏠 Home Charger Guide
    </a>
  </div>

  <p class="text-center text-xs mt-4" style="color:#94A3B8">Charging time estimates are 20%→100% (80% SoC). Actual times vary by ambient temperature and charger load.</p>
