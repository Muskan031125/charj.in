...
<form method="post" action="/admin/settings/save" class="space-y-6" x-data="settingsForm()" enctype="multipart/form-data">
  <?= csrf_field() ?>
  ...
  <div class="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
    <div class="flex items-center gap-3 bg-slate-50 border-b border-slate-200 px-6 py-4">
      ...
      <div>
        <h2 class="text-sm font-bold text-slate-900">Homepage Images</h2>
        <p class="text-xs text-slate-500">Every photo used on the homepage — hero banner, featured visual, EV Lifestyle slider, and Why Choose EV slider</p>
      </div>
    </div>
    <div class="p-6 grid grid-cols-1 gap-6 sm:grid-cols-3">
      ...
      <?php
      $homepageImageFields = [
        ['key' => 'homepage_hero_image',            'file' => 'homepage_hero_image_file',            'label' => 'Top Hero Banner',                          'hint' => 'Landscape, e.g. 1920×1080'],
        ['key' => 'homepage_featured_visual_image', 'file' => 'homepage_featured_visual_image_file', 'label' => '"Future of Mobility" Section',             'hint' => 'Landscape, e.g. 1600×900'],
        ['key' => 'homepage_lifestyle_image_1',     'file' => 'homepage_lifestyle_image_1_file',     'label' => 'EV Lifestyle — Slide 1 ("Smart Living")',  'hint' => 'Landscape, e.g. 1200×900'],
        ['key' => 'homepage_lifestyle_image_2',     'file' => 'homepage_lifestyle_image_2_file',     'label' => 'EV Lifestyle — Slide 2 ("Premium Drives")', 'hint' => 'Landscape, e.g. 1200×900'],
        ['key' => 'homepage_benefit_image_1',       'file' => 'homepage_benefit_image_1_file',       'label' => 'Why Choose EV — Slide 1 ("Save Money")',   'hint' => 'Landscape, e.g. 800×600'],
        ['key' => 'homepage_benefit_image_2',       'file' => 'homepage_benefit_image_2_file',       'label' => 'Why Choose EV — Slide 2 ("Zero Emissions")', 'hint' => 'Landscape, e.g. 800×600'],
        ['key' => 'homepage_benefit_image_3',       'file' => 'homepage_benefit_image_3_file',       'label' => 'Why Choose EV — Slide 3 ("Silent & Smooth")', 'hint' => 'Landscape, e.g. 800×600'],
      ];
      foreach ($homepageImageFields as $f):
        $current = $s($f['key']);
      ?>
      <div>
        <?php if ($current): ?>
        <div class="mb-3 rounded-xl overflow-hidden border border-slate-200 aspect-video bg-slate-50">
          <img src="<?= esc($current) ?>" alt="Current <?= esc($f['label']) ?>" class="w-full h-full object-cover">
        </div>
        <?php else: ?>
        <div class="mb-3 rounded-xl border border-dashed border-slate-200 aspect-video bg-slate-50 flex items-center justify-center text-xs text-slate-400">No image set</div>
        <?php endif; ?>
        <label for="<?= esc($f['file']) ?>" class="block text-sm font-medium text-slate-700 mb-1.5"><?= esc($f['label']) ?></label>
        <input type="file" id="<?= esc($f['file']) ?>" name="<?= esc($f['file']) ?>" accept="image/*"
               class="w-full rounded-xl border border-slate-200 px-3 py-2 text-xs text-slate-900 file:mr-3 file:rounded-lg file:border-0 file:bg-emerald-50 file:px-3 file:py-1.5 file:text-xs file:font-semibold file:text-emerald-700 hover:file:bg-emerald-100">
        <p class="mt-1.5 text-xs text-slate-400"><?= esc($f['hint']) ?></p>
      </div>
      <?php endforeach; ?>

    </div>
    <div class="px-6 pb-5">
      <p class="text-xs text-slate-400">Uploading a new image replaces the current one after you click Save Settings below.</p>
    </div>
  </div>
