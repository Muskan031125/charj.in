...
<form method="post" action="<?= esc($formAction) ?>" enctype="multipart/form-data" class="space-y-6" x-data="brandForm()">
  <?= csrf_field() ?>
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
          ...
          <div class="sm:col-span-2">
            ...
            <div class="mb-2 flex items-center gap-2">
              <img src="<?= esc($brand['logo']) ?>" class="h-10 w-10 object-contain rounded-lg border border-slate-200 bg-slate-50 p-1">
              <span class="text-xs text-slate-400">Current logo</span>
            </div>
            <?php endif; ?>
            <input type="file" id="logo_file" name="logo_file" accept="image/*"
                   class="w-full text-sm text-slate-600 file:mr-3 file:rounded-lg file:border-0 file:bg-slate-100 file:px-3 file:py-2 file:text-xs file:font-bold mb-2">
            <div class="flex items-center gap-2 mb-2">
              <div class="flex-1 h-px bg-slate-200"></div>
              <span class="text-[10px] font-semibold text-slate-400">OR PASTE URL</span>
              <div class="flex-1 h-px bg-slate-200"></div>
            </div>
            <input type="url" id="logo_url" name="logo_url"
                   value="<?= esc(old('logo_url', $brand['logo'] ?? '')) ?>"
                   placeholder="https://cdn.example.com/logos/tata.png"
                   class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none">
            <p class="mt-1 text-xs text-slate-400">Upload a file, or paste a direct image URL (PNG/SVG preferred, transparent background). Uploading a file takes priority.</p>
          </div>
        ...
        <div class="space-y-5">
          ...
          <div>
            ...
            <select id="status" name="status"
                    class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none">
              <?php $statusVal = old('status', $brand['status'] ?? 'draft'); ?>
