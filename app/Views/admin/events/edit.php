    ...
    <div class="grid grid-cols-1 gap-5 lg:grid-cols-3">
            ...
            <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="title" value="<?= esc(old('title', $event['title'])) ?>" required
                    ...
                    <div>
                        ...
                        <input type="text" name="slug" value="<?= esc(old('slug', $event['slug'])) ?>"
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="5" required
                                  class="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20"><?= esc(old('description', $event['description'])) ?></textarea>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="mb-1 block text-xs font-semibold text-slate-600">City</label>
                            <input type="text" name="city" value="<?= esc(old('city', $event['city'] ?? '')) ?>"
                        ...
                        <div>
                            ...
                            <input type="text" name="organizer" value="<?= esc(old('organizer', $event['organizer'] ?? '')) ?>"
                                   class="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm focus:border-emerald-500 focus:outline-none">
                        </div>
                    </div>
                    <div>
                        <label class="mb-1 block text-xs font-semibold text-slate-600">Venue Address</label>
                        <input type="text" name="venue_address" value="<?= esc(old('venue_address', $event['venue_address'] ?? '')) ?>"
                               class="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm focus:border-emerald-500 focus:outline-none">
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="mb-1 block text-xs font-semibold text-slate-600">Start Date <span class="text-red-500">*</span></label>
                            <input type="date" name="start_date" value="<?= esc(old('start_date', substr($event['start_date'] ?? '', 0, 10))) ?>" required
                                   class="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm focus:border-emerald-500 focus:outline-none">
                        </div>
                        <div>
                            <label class="mb-1 block text-xs font-semibold text-slate-600">End Date</label>
                            <input type="date" name="end_date" value="<?= esc(old('end_date', substr($event['end_date'] ?? '', 0, 10))) ?>"
                                   class="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm focus:border-emerald-500 focus:outline-none">
                        </div>
                    </div>
                    <div>
                        <label class="mb-1 block text-xs font-semibold text-slate-600">Registration URL</label>
                        <input type="url" name="registration_url" value="<?= esc(old('registration_url', $event['registration_url'] ?? '')) ?>"
                               class="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm focus:border-emerald-500 focus:outline-none">
                    </div>
                    <div>
                        <label class="mb-1 block text-xs font-semibold text-slate-600">Banner Image URL</label>
                        <input type="url" name="banner_image" value="<?= esc(old('banner_image', $event['banner_image'] ?? '')) ?>"
                               class="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm focus:border-emerald-500 focus:outline-none">
                    </div>
                </div>
            </div>
        ...
        <div class="space-y-5">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select name="event_type" class="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm focus:border-emerald-500 focus:outline-none">
                            ...
                            <option value="<?= $t ?>" <?= ($event['event_type'] ?? '') === $t ? 'selected' : '' ?>><?= ucwords(str_replace('_',' ',$t)) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div>
            ...
            <div class="flex gap-2">
                ...
                <a href="/admin/events/delete/<?= $event['id'] ?>" onclick="return confirm('Delete?')"
