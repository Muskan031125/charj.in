                ...
                <tr class="hover:bg-slate-50">
                    <td class="px-4 py-3">
                        <?php if (!empty($v['image_url'])): ?>
                            <img src="<?= esc($v['image_url']) ?>" alt="<?= esc($v['name']) ?>"
                                 class="h-12 w-16 rounded-lg object-cover ring-1 ring-slate-200 bg-slate-50">
                        <?php else: ?>
                            <div class="h-12 w-16 rounded-lg bg-slate-100 flex items-center justify-center text-slate-300 text-xs ring-1 ring-slate-200">—</div>
                        <?php endif; ?>
                    </td>
                    ...
                    <td class="px-4 py-3 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <a href="/admin/vehicles/edit/<?= $v['id'] ?>"
                               class="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 transition hover:bg-slate-100">
                                Edit
                            </a>
                            <a href="/admin/vehicles/delete/<?= $v['id'] ?>"
                               onclick="return confirm('Delete <?= esc(addslashes($v['name'])) ?>? This cannot be undone.')"
