...
// $recordKey lets a form field whose POST name differs from the real DB column (e.g. form
// field "motor_power" but real column "motor_power_kw") still pre-fill correctly when editing.
function fv(string $key, array $record = [], string $default = '', ?string $recordKey = null): string {
    $val = old($key);
    $rk  = $recordKey ?? $key;
    return esc($val !== null ? $val : ($record[$rk] ?? $default));
}
function fvSel(string $key, string $val, array $record = [], ?string $recordKey = null): string {
    $rk  = $recordKey ?? $key;
    $cur = old($key) ?? ($record[$rk] ?? '');
      ...
      <div x-show="active === 2" x-cloak class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        ...
        <div class="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <input type="number" name="motor_power" value="<?= fv('motor_power', $v, '', 'motor_power_kw') ?>" min="0" step="0.1"
                   class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none">
          </div>

          <div>
            <label class="block text-sm font-semibold text-slate-700 mb-1.5">Motor Torque (Nm)</label>
            <input type="number" name="motor_torque" value="<?= fv('motor_torque', $v, '', 'torque_nm') ?>" min="0" step="0.1"
                   class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none">
          </div>

          <div>
            <label class="block text-sm font-semibold text-slate-700 mb-1.5">Top Speed (km/h)</label>
            <input type="number" name="top_speed" value="<?= fv('top_speed', $v, '', 'top_speed_kmph') ?>" min="0"
                   class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none">
          </div>

          <div>
            <label class="block text-sm font-semibold text-slate-700 mb-1.5">0&#8211;100 km/h (sec)</label>
            <input type="number" name="acceleration_0_100" value="<?= fv('acceleration_0_100', $v, '', 'acceleration_0_60') ?>" min="0" step="0.1"
                   class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none">
          </div>
      ...
      <div x-show="active === 3" x-cloak class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        ...
        <div class="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <input type="text" name="charging_time_normal" value="<?= fv('charging_time_normal', $v, '', 'charging_time_ac') ?>"
          ...
          <div>
            ...
            <input type="text" name="charging_time_fast" value="<?= fv('charging_time_fast', $v, '', 'charging_time_dc') ?>"
          ...
          <div>
            ...
            <select name="fast_charging_type"
              ...
              <option value="CCS2"        <?= fvSel('fast_charging_type', 'CCS2', $v, 'charging_connector_type') ?>>CCS2 (DC)</option>
              <option value="CHAdeMO"     <?= fvSel('fast_charging_type', 'CHAdeMO', $v, 'charging_connector_type') ?>>CHAdeMO (DC)</option>
              <option value="GB/T"        <?= fvSel('fast_charging_type', 'GB/T', $v, 'charging_connector_type') ?>>GB/T</option>
              <option value="Proprietary" <?= fvSel('fast_charging_type', 'Proprietary', $v, 'charging_connector_type') ?>>Proprietary</option>
              <option value="Type-2"      <?= fvSel('fast_charging_type', 'Type-2', $v, 'charging_connector_type') ?>>Type 2 (AC)</option>
            </select>
          </div>
      ...
      <div x-show="active === 4" x-cloak class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        ...
        <div class="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <input type="number" name="kerb_weight" value="<?= fv('kerb_weight', $v, '', 'weight_kg') ?>" min="0"
                   class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none">
          </div>

          <div>
            <label class="block text-sm font-semibold text-slate-700 mb-1.5">Ground Clearance (mm)</label>
            <input type="number" name="ground_clearance" value="<?= fv('ground_clearance', $v, '', 'ground_clearance_mm') ?>" min="0"
                   class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none">
          </div>

          <div>
            <label class="block text-sm font-semibold text-slate-700 mb-1.5">Boot Space (litres)</label>
            <input type="number" name="boot_space" value="<?= fv('boot_space', $v, '', 'boot_space_litres') ?>" min="0"
                   class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none">
          </div>
...
<?php if ($isEdit): ?>
<!-- Gallery & Interior Photos — separate form/section, since a real vehicle_images upload
     needs its own <form>, and forms cannot be nested inside the main vehicle-edit <form> above. -->
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 -mt-2 mb-8">
  <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
    <h2 class="text-sm font-bold uppercase tracking-wide text-slate-500 mb-4">Gallery &amp; Interior Photos</h2>

    <?php if (!empty($galleryImages)): ?>
    <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3 mb-5">
      <?php foreach ($galleryImages as $img): ?>
      <div class="relative group rounded-lg overflow-hidden border border-slate-200 bg-slate-50">
        <img src="<?= esc($img['image_url']) ?>" class="w-full h-20 object-cover" loading="lazy">
        <span class="absolute top-1 left-1 text-[9px] font-bold uppercase px-1.5 py-0.5 rounded bg-slate-900/70 text-white"><?= esc($img['image_type']) ?></span>
        <a href="<?= base_url('admin/vehicles/' . $vehicle['id'] . '/images/delete/' . $img['id']) ?>"
           onclick="return confirm('Remove this photo?')"
           title="Remove"
           class="absolute top-1 right-1 w-5 h-5 rounded-full bg-red-500 text-white text-xs leading-5 text-center opacity-0 group-hover:opacity-100 transition-opacity">&times;</a>
      </div>
      <?php endforeach; ?>
    </div>
    <?php else: ?>
    <p class="text-xs text-slate-400 mb-5">No gallery or interior photos yet — add some below.</p>
    <?php endif; ?>

    <form method="post" action="<?= base_url('admin/vehicles/' . $vehicle['id'] . '/images/upload') ?>"
          enctype="multipart/form-data" class="flex flex-wrap items-end gap-3 pt-4" style="border-top:1px solid #E2E8F0">
      <?= csrf_field() ?>
      <div>
        <label class="block text-xs font-semibold text-slate-600 mb-1">Photo type</label>
        <select name="image_type" class="rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 outline-none">
          <option value="gallery">Gallery</option>
          <option value="interior">Interior</option>
        </select>
      </div>
      <div>
        <label class="block text-xs font-semibold text-slate-600 mb-1">Choose file</label>
        <input type="file" name="image_file" accept="image/*" required
               class="text-sm text-slate-600 file:mr-3 file:rounded-lg file:border-0 file:bg-slate-100 file:px-3 file:py-2 file:text-xs file:font-bold">
      </div>
      <button type="submit" class="rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold px-4 py-2 transition-colors">
        Upload Photo
      </button>
    </form>
  </div>
