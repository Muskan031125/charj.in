        ...
        <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
            ...
            <div class="mt-4 space-y-3">
                ...
                foreach ($leadsByType as $type => $count):
                    $pct = $totalLeadsChart > 0 ? round(($count / $totalLeadsChart) * 100) : 0;
                    $label = $typeLabels[$type] ?? ucwords(str_replace('_', ' ', $type));
                    $color = $barColors[$type] ?? 'bg-slate-400';
                ?>
                <div>
                    <div class="mb-1 flex items-center justify-between">
                        <span class="text-xs font-medium text-slate-700"><?= esc($label) ?></span>
                        <span class="text-xs font-bold text-slate-900"><?= fmtNum($count) ?> <span class="font-normal text-slate-400">(<?= $pct ?>%)</span></span>
                    </div>
