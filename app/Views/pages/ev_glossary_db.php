...
/* Term card */
.term-card {
  background:#fff; border-radius:1.25rem; padding:1.25rem 1.4rem;
  border:1.5px solid #EAFFF4; transition:all .2s cubic-bezier(.22,1,.36,1);
  box-shadow:0 1px 4px rgba(0,0,0,.04);
}
.term-card:hover { border-color:rgba(0,168,150,.4); box-shadow:0 8px 24px rgba(0,168,150,.12); transform:translateY(-2px); }

/* Category icon badge — left of term name */
.cat-icon {
  width:2.25rem; height:2.25rem; border-radius:.75rem; flex-shrink:0;
  display:flex; align-items:center; justify-content:center; font-size:1.05rem;
}
.cat-icon-battery  { background:#fef3c7; }
.cat-icon-charging { background:rgba(0,168,150,.1); }
.cat-icon-performance { background:#dcfce7; }
.cat-icon-policy   { background:#ede9fe; }
.cat-icon-finance  { background:#fce7f3; }
.cat-icon-general  { background:#F0FFF9; }
...
.cat-charging { background:rgba(0,168,150,.1); color:#007A6E; }
...
.cat-general  { background:#F0FFF9; color:#475569; }
...
.ltr-pill:hover { background:#F0FFF9; color:#00A896; }
.ltr-pill.active { background:linear-gradient(135deg,#00A896,#007A6E); color:#fff; border-color:transparent; box-shadow:0 2px 8px rgba(0,168,150,.4); }
...
.filter-pill:hover { border-color:rgba(0,168,150,.4); color:#00A896; }
.filter-pill.active { background:linear-gradient(135deg,#00A896,#007A6E); color:#fff; border-color:transparent; box-shadow:0 2px 8px rgba(0,168,150,.35); }
...
<!-- ═══ HERO ═══ -->
<section class="hero-sm relative overflow-hidden pt-28 md:pt-32 pb-14 px-4" style="background:linear-gradient(135deg,#F0FFF9,#EAFFF4,#F7FFFE)">
  ...
  <div class="relative max-w-3xl mx-auto text-center">
    <div class="hero-badge inline-flex items-center gap-2 rounded-full px-3.5 py-1.5 text-xs font-bold uppercase tracking-widest mb-6"
         style="background:rgba(0,168,150,.10);border:1px solid rgba(0,168,150,.25);color:#00A896">
      📖 <?= $total ?>+ EV Terms Explained for India
    </div>
    <h1 class="text-4xl sm:text-5xl lg:text-6xl font-black tracking-tight mb-4 leading-tight" style="color:#0F172A">
      EV <span style="background:linear-gradient(135deg,#00A896,#00E676);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text">Glossary</span>
    </h1>
    <p class="hero-desc text-base sm:text-lg max-w-xl mx-auto mb-7" style="color:#475569">
      Every electric vehicle term — from kWh to FAME II, BMS to V2G — explained in plain English for Indian buyers.
    </p>

    <!-- Search bar -->
    <div class="relative max-w-lg mx-auto">
      <svg class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5" style="color:#94A3B8" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
      <input x-model="searchQ" @input="onSearch()" type="text"
             placeholder="Search terms — kWh, BMS, FAME II, V2G..."
             class="w-full rounded-2xl pl-12 pr-12 py-4 text-sm focus:outline-none focus:ring-2 transition-all"
             style="background:#fff;border:1.5px solid #d7f5ec;color:#0F172A;--tw-ring-color:rgba(0,168,150,.15)">
    ...
    <!-- Search result count -->
    <p class="text-xs mt-3 h-4" style="color:#94A3B8"
       x-show="searchQ"
...
<!-- ═══ STICKY CONTROLS ═══ -->
<div class="sticky top-16 md:top-28 z-40 bg-white/95 backdrop-blur border-b border-slate-100 shadow-sm">
...
<div class="max-w-6xl mx-auto px-4 py-10">
  ...
  <div x-show="filteredTerms.length===0" x-cloak class="text-center py-20">
    ...
    <button @click="setLetter('all');setFilter('all');searchQ='';onSearch()"
            class="text-white font-bold px-5 py-2.5 rounded-xl text-sm transition-all hover:-translate-y-0.5"
    ...
    <div class="mb-10">
      ...
      <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
        <template x-for="(t, i) in group.terms" :key="t.term">
          <div class="term-card animate-in flex items-start gap-3" :style="`animation-delay:${i*40}ms`" x-data="{open:false}">
            <div class="cat-icon" :class="`cat-icon-${t.category}`" x-text="catIcon(t.category)"></div>
            <div class="flex-1 min-w-0">
              <div class="flex items-start justify-between gap-3">
                ...
                <button @click="open=!open" class="text-slate-400 flex-shrink-0 transition-colors" style="color:#94A3B8" onmouseover="this.style.color='#00A896'" onmouseout="this.style.color='#94A3B8'">
              ...
              <span class="inline-block text-[10px] font-bold uppercase tracking-wide px-2.5 py-1 rounded-full mt-1.5"
  ...
  <!-- CTA -->
  <div x-show="filteredTerms.length > 0" class="mt-14 rounded-3xl overflow-hidden" style="background:linear-gradient(135deg,#0f172a 0%,#0A3D2B 60%,#052e16 100%)">
      ...
      <div class="relative">
        ...
        <div class="flex flex-col sm:flex-row gap-3 justify-center">
          <a href="<?= base_url('find-my-ev') ?>"
             class="inline-flex items-center justify-center gap-2 font-bold px-7 py-3.5 rounded-full text-sm transition-all duration-200 text-white hover:-translate-y-0.5"
             style="background:linear-gradient(135deg,#00A896,#00C9B1);box-shadow:0 4px 16px rgba(0,168,150,.3)">
            Find My EV with AI ⚡
          </a>
...
function glossaryApp() {
  return {
    ...
    catIcon(cat) {
      return { battery:'🔋', charging:'⚡', performance:'🏎️', policy:'📋', finance:'💰', general:'💡' }[cat] || '💡';
    },
